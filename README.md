...
3. **First launch:** because the app isn't signed with a paid Apple Developer certificate, macOS blocks it the first time.
   - **On macOS 13–15 (Ventura, Sonoma, Sequoia):** **right-click** (or Control-click) the app → **Open** → **Open** again in the dialog. Or go to **System Settings → Privacy & Security**, scroll down, and click **Open Anyway**.
   - **On macOS 26 (Tahoe):** the steps above often *don't* work — Tahoe refuses unsigned downloaded apps with a *"can't be opened"* error. Use Terminal once instead:
     ...
     5. Try to open the app (double-click). On some Tahoe setups this is enough. If it still won't open, also do the classic **right-click (Control-click) the app → Open → Open**, or **System Settings → Privacy & Security → Open Anyway** — the `xattr` step above makes that route work. You only need to do this once.
